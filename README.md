...
| **招投标公告（国内免费·直接带电话）** | `server/sources/tender.js` | **已落实·实时生效（默认）** | 实时聚合中国政府采购网公开招标/中标/成交公告（栏目页服务端渲染，避开 search 子域限流），提炼 **采购人 + 联系人 + 座机/手机 + 邮箱 + 预算**。**零成本、且直接解决企查查 886 不返电话的痛点**。默认在「刷新数据源」时启用真实聚合（`TENDER_LIVE=0` 可关）；单次最多抓 `TENDER_MAX` 条详情（默认 20，含 600ms 限速守 robots） |
...
- **实时免费信号**：招投标（中国政府采购网）默认在刷新时实时抓取，直接带采购人电话，是当前最稳的国内真实源。
- **规模化工商源**：企查查（`QCC_APPKEY`/`QCC_SECRET`）填 key 后即启用，补充公司/法人/地区；其 886 接口默认不返电话，故联系方式以招投标/ enrich 互补。
- **说明**：ccgp 关键词搜索子域与第三方聚合站（cebpubservice 等）均有反爬/限流，故采用「栏目页抓取 + 详情页提炼」的稳健路径；单日最新批次中阀门招标数量有限，刷新即取当日公开数据。

## 下一步

1. 用户填入**企查查 key** → 国内工商数据规模化落地（与招投标交叉验证，线索自动升权）。
2. 国外：接 **Companies House（英国免费 API）** 等免费源，继续补齐海外真实数据。
3. 替换 `server/store.js` 为真实数据库，并加固密码强度 / 限流 / 招投标抓取频控。
